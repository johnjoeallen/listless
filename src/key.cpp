...
KeyCode alt_key(char c) {
    ...
    // Scan codes for Alt+1..Alt+9, matching VKALT_1..VKALT_9. There is
    // no VKALT_0 in the original.
    static constexpr std::array<int, 9> kAltDigit = {
    ...
    if (c >= '1' && c <= '9') {
        std::size_t index = static_cast<std::size_t>(c - '1');
